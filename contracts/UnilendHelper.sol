...
interface IUnilendV2Position {
    ...
    function getNftId(
        address _pool,
        address _user
    ) external view returns (uint nftID);
}
...
interface IUnilendV2Pool {
    ...
    function userBalanceOftoken0(
        uint _nftID
    ) external view returns (uint _lendBalance0, uint _borrowBalance0);

    function userBalanceOftoken1(
        uint _nftID
    ) external view returns (uint _lendBalance1, uint _borrowBalance1);
    ...
    function userSharesOftoken0(
        uint _nftID
    ) external view returns (uint _lendShare0, uint _borrowShare0);

    function userSharesOftoken1(
        uint _nftID
    ) external view returns (uint _lendShare1, uint _borrowShare1);
    ...
    function userHealthFactor(
        uint _nftID
    ) external view returns (uint256 _healthFactor0, uint256 _healthFactor1);
...
contract helper {
    ...
    function getPoolFullData(
        ...
        (_out._totalLendShare0, _out._totalBorrowShare0, _out._totalBorrow0) = p
            .token0Data();
        (_out._totalLendShare1, _out._totalBorrowShare1, _out._totalBorrow1) = p
            .token1Data();

        _out._interest0 = p.getInterestRate(
            _out._totalBorrow0,
            _out._token0Liquidity
        );
        _out._interest1 = p.getInterestRate(
            _out._totalBorrow1,
            _out._token1Liquidity
        );
        ...
        if (_user != address(0)) {
            ...
            if (_nftID > 0) {
                (
                    ...
                    _out._lendShare1,
                    _out._borrowShare1
                ) = p.userSharesOftokens(_nftID);
                (
                    _out._lendBalance0,
                    _out._borrowBalance0,
                    _out._lendBalance1,
                    _out._borrowBalance1
                ) = p.userBalanceOftokens(_nftID);
                (_out._healthFactor0, _out._healthFactor1) = p.userHealthFactor(
                    _nftID
                );
            }
        }
    ...
    function getPoolData(
        address _pool
    ) external view returns (outData memory _out) {
        IUnilendV2Pool p = IUnilendV2Pool(_pool);
